...
XO <- function(str) {
  ...
  sum(str_lower == "x") == sum(str_lower == "o")
}
result <- XO("ooxx")
print(result)
...
get_char <- function(number) {
  return(as.character(rawToChar(as.raw(number))))
}
```
...
solution <- function(number) {
  multiples_of_3 <- sum((1:(number-1)) %% 3 == 0 & (1:(number-1)) %% 5 != 0)
  multiples_of_5 <- sum((1:(number-1)) %% 3 != 0 & (1:(number-1)) %% 5 == 0)
  multiples_of_3_and_5 <- sum((1:(number-1)) %% 3 == 0 & (1:(number-1)) %% 5 == 0)
  
  return(c(multiples_of_3, multiples_of_5, multiples_of_3_and_5))
}
...
```
```{r}
plot(women$height, women$weight, xlab = "Height (inches)", ylab = "Weight (pounds)", main = "Height vs Weight")
```
Basically, viewing the scaterplot we can see that the taller a woman is the more she will. 
